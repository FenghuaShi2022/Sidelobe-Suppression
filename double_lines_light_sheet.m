...
NA_outer=0.65;%outer NA of the annular ring
radius_outer=focal_length*tan(asin(NA_outer));%outer annular ring radius
NA_inner=0.57;%inner NA of the annular ring
radius_inner=focal_length*tan(asin(NA_inner));%inner annular ring radius
NA_x_lines=0.6;%NA location of the double line beams at the rear pupil plane
x_lines=focal_length*tan(asin(NA_x_lines)); % x postion of the double line beams at the rear pupil plane

length_1=2*sqrt(radius_outer^2-x_lines^2);%length of the a1 line beams: L1
ratio=3/5; % ratio of L2/L1 obtained by the theoretical derivation
length_2=length_1*ratio;%length of the a2 line beams: L2
...
%        condition(j,i)=((x(i)^2+z(j)^2)<=radius_outer^2)&&((x(i)^2/1^2+z(j)^2/1)>=radius_inner^2)&&(abs(abs(x(i))-x_lines)<=width_2/2)&&abs(z(j))<=length_2/2;    % a2-line beams.
        condition(j,i)=((x(i)^2+z(j)^2)<=radius_outer^2)&&((x(i)^2/1^2+z(j)^2/1)>=radius_inner^2)&&((abs(abs(x(i))-x_lines)<=width_1/2)||((abs(abs(x(i))-x_lines)<=(width_1+width_2)/2)&&(abs(z(j))<=length_2/2)));    % a3-line beams
       if  condition(j,i)
...
%%%%%%%%%%%
tic
y_imaging=linspace(focal_length*2-20*wavelength, focal_length*2+20*wavelength,2*20*10);
z_imaging=linspace(-20*wavelength, 20*wavelength,2*20*10);
x_imaging=0;
...
%%%%%%% calculation of illumination uniformity within the effective propagation distance: +-5 um from the focal point 
for m=1:length(y_imaging)
    if (y_imaging(m)-2*focal_length)>-5e-6
...
%%%%%%%%%%%
tic
x_imaging=linspace(-6*wavelength, 6*wavelength,2*6*10);
z_imaging=linspace(-6*wavelength, 6*wavelength,2*6*10);
...
kz_a2_max=kz_a1_max*ratio; 
for i=1:length(kz)
       if (abs(kz(i))<=kz_a2_max)
...
F_z_a3=(F_z_a1+F_z_a2/ratio); %F_z_a3: focal electric field z-profile in real space
